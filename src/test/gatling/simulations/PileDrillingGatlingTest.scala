...
/**
 * Performance test for the PileDrilling entity.
 */
class PileDrillingGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the PileDrilling entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all pileDrillings")
            .get("/api/pileDrillings")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            ...
            .pause(10)
            .repeat(5) {
                exec(http("Get created pileDrilling")
                .get("${new_pileDrilling_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created pileDrilling")
            .delete("${new_pileDrilling_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
